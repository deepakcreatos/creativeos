        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full animate-in fade-in duration-500 space-y-8">
            <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-2">Review generated assets, request AI revisions, or lock for scheduling.</p>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="p-8">
                            <div className="flex gap-6 items-start">
                                <div className="w-32 h-32 bg-slate-100 rounded-xl border border-slate-200 flex items-center justify-center flex-shrink-0">
                                    <span className="text-slate-400 text-sm font-bold">Generated<br/>Image</span>
                                </div>
                                <div className="space-y-4 flex-1">
                                    <div>
                                        <h3 className="font-bold text-slate-900">Winter Collection Launch #4</h3>
                                        <p className="text-sm text-slate-500">Instagram Feed Post • Campaign: Holiday Q4</p>
                                    </div>
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm h-full flex flex-col">
                        <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                            <h3 className="font-bold text-slate-700">Action Stream</h3>
                        </div>
                            ...
                            {actionLog.length === 0 ? (
                                ...
                                actionLog.map((log, i) => (
                                    <div key={i} className={`p-3 rounded-lg border text-sm animate-in slide-in-from-left-2 ${log.type === 'APPROVE' ? 'bg-green-50 border-green-100' : 'bg-amber-50 border-amber-100'}`}>
                                        ...
                                        {log.type === 'REVISE' && (
                                            <p className="text-slate-600 text-xs mt-1 italic">"{log.feedback}"</p>
                                        )}
                                        <div className="mt-2 text-[10px] bg-white/50 p-2 rounded border border-white/50 font-mono text-slate-500 overflow-hidden">
