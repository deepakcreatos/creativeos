        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full animate-in fade-in duration-500 space-y-8">
            <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-2">Real-time cost processing and automated invoicing.</p>
                </div>
                <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-3 text-slate-500 mb-2 font-bold text-sm">
                        <TrendingUp size={16} className="text-indigo-500" /> Current Month Spend
                    </div>
                    <div className="text-3xl font-bold text-slate-900">$2,450.00</div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div className="text-3xl font-bold text-slate-900">Agency Pro</div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mt-8">
                <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                    <h3 className="font-bold text-slate-900">Recent Invoices (System Logs)</h3>
                </div>
                {invoices.length === 0 ? (
                    ...
                    <div className="divide-y divide-slate-100">
                        {invoices.map((inv, i) => (
                            <div key={i} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors animate-in slide-in-from-top-2">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center font-bold">
                                        <Receipt size={20} />
                                    </div>
                                    <div>
                                        <div className="font-bold text-sm text-slate-900">Invoice #{inv.transactionId?.split('-')[0] || 'INV-100'}</div>
                                        <div className="text-xs text-slate-500">System Trigger: Billing Processing</div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-6">
                                    <div className="text-right">
                                        <div className="font-bold text-slate-900">${inv.amount?.toFixed(2) || '450.00'}</div>
